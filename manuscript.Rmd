...
author:
  - Daniel Hammarström^1,2,£^*
  - Sjur J. Øfsteng^2,£^
  - Nicolai B. Jacobsen^1^
  - Krister B. Flobergseter^1^
  - Bent R. Rønnestad^1^
  - Stian Ellefsen^1,3^
date: ''
output:
...
**Aim**
To describe the time course of ribosome accumulation in response to resistance training with constant and variable volume and relate RNA accumulation to total UBF levels and muscle growth. 
**Methods**
Participants allocated to a training group (n=11) performed unilateral knee extension with constant (6 sets per session) and variable volume (6, 3, and 9 sets in sessions 1-4, 5-8, and 9-12, respectively) allocated to either leg. Muscle biopsies (m. vastus lateralis) were sampled in the training group before and 48 hrs after the first session and 48 hrs after sessions 4, 5, 8, 9, and 12, and after eight days of de-training. Muscle samples were also obtained from a non-training control group (n=7) at baseline and after 48 hrs and 3-5 weeks. Muscle specimens were assayed for total RNA, ribosomal RNA (rRNA), ribosomal protein S6 (rpS6), and upstream binding factor (UBF) protein content. Muscle thickness (ultrasound) and strength (isokinetic and isometric knee extension) were measured before and after resistance training or control periods.
**Results**
Resistance training led to muscle growth (0.84 mm, 95% CI: [0.05, 1.64] vs. control) with concomitant increases in total RNA, rRNA, UBF and rpS6 protein. Total RNA increased most rapidly in response to the first four sessions (8.6% [5.5, 11.7] per session), followed by a plateau and peak values (49.5% [34.2, 66.5] above baseline) after session 8. UBF protein levels were associated with total RNA levels after controlling for time, and increases in total RNA levels predicted RT-induced muscle hypertrophy. After de-training, total RNA and rRNA decreased without muscle mass changes indicating halted biosynthesis of ribosomes. In conclusion, ribosomes accumulate in the initial phase of RT and is sensitive to training cessation. The rate of accumulation predicts muscle hypertrophy 


Increased ribosomal density is a phenomenon observed in response to resistance training (RT) supporting elevated. A time course of RT-induced accumulation of ribosomes has not been described. We mapped markers of ribosomal biogenesis (total RNA, ribosomal RNA (rRNA), ribosomal protein S6 (rpS6), and upstream binding factor (UBF) protein content) in response to twelve RT sessions. Additionally, we investigated the effect of fluctuations in training volume on these markers and the relationship between RNA accumulation and muscle growth measured as muscle thickness (ultrasound). Eighteen participants were allocated to either a training (TRAIN, *n*=11) or a control (CTRL, *n*=7) group. In TRAIN, participants performed unilateral knee extension with either constant (CONST, 6 sets) or a variable volume (VAR, 6, 3, and 9 sets in sessions 1-4, 5-8, and 9-12, respectively). Muscle biopsies were sampled from *m vastus lateralis* in TRAIN before and 48 hrs after the first session and 48 hrs after sessions 4, 5, 8, 9, and 12, and after eight days of de-training. Control biopsies were sampled in CTRL at baseline and after 48 hrs and 3-5 weeks. RT led to muscle growth in TRAIN compared to CTRL and concomitant increases in total RNA, rRNA, including precursor rRNA as well as UBF and rpS6 protein through gradual increase throughout the training period, confirmed in comparison to CTRL. Total RNA increased in a curve-linear fashion, most rapidly in response to the first four sessions  (8.6%, 95% CI: [5.5, 11.7] per session), followed by a plateau and peak values (49.5% [34.2, 66.5] above baseline) after Session 8. UBF protein levels explained total RNA levels after controlling for time, and increases in total RNA levels predicted RT-induced muscle hypertrophy. After de-training, total RNA and specific rRNA species decreased without muscle mass changes indicating reduced concentrations and biosynthesis of ribosomes. These results underline a determinant role for ribosomal biogenesis in RT-induced muscle hypertrophy and that ribosomal biogenesis is sensitive to training cessation.  
...
read_excel("./data/tr010_age_participant.xlsx") %>%
  ...
  mutate(variable =  factor(variable,
                          ...
                          labels = c("n", "Age (yrs)", "Body mass, (kg)", "Stature (cm)", "Body mass index (kg/m\\textsuperscript{2})", "Body fat (\\%)"))) %>%
  
  dplyr::select(variable, F_experiment, M_experiment, F_control, M_control) %>%

  kable(caption = "Participant characteristics.", 
        booktabs = TRUE, 
        col.names = c("", "Female", "Male", "Female", "Male"),
        escape = FALSE,
        format = "latex") %>%
  add_header_above(c(" ", "Experimental group" = 2, "Control group" = 2))
...
dplyr::select(Symbol = symbol_ext, 'Transcript name' = full_name, Sequence = seq, 'Mean Cq (SD) and efficiency' = performance) %>%
    filter(!(Symbol %in% c("RPL32", "MYH7", "MYH2", "MYH1")))

primer_table %>%
  kable(format = "latex", 
        caption = "Primer sequences and average performance.",
        booktabs = TRUE) %>%
...
```{r figure1, fig.height=(23*0.75)/2.54, fig.width=17.8/2.54, fig.align="center", fig.cap="Study design (A) observed training loads in response to Constant and Variable volume protocols (B) and training outcomes (muscle strength, C; muscle thickness D).", echo=FALSE}
...
A single session of RT (Session 1) led to robust increases in pre-rRNA 47S ETS and 45S ETS abundance per unit tissue weight, measured as changes from baseline to 48 hrs after exercise within TRAIN (Figure 3B), as well as compared to CTRL (Figure 3C).
...
This general pattern of rRNA expression was reflected by total RNA expression per unit tissue weight, which increased robustly and steadily in TRAIN throughout the initial part of the intervention (Figure 3E and G), leading to robust increase compared to CTRL after Session 12 (Figure 3F), followed by decreased levels after de-training (`r detrain_rna`).
...
```{r figure3, fig.height=(23*0.75)/2.54, fig.width=17.8/2.54, fig.align="center", fig.cap="Total RNA and ribosomal RNA subspecies in response to trainin. Total RNA increased compared to non-training controls to post-training (12 sessions) and tended to normalize after de-training (A). There were no robust differences between different volume conditions in the training group (A). B shows primer locations targeting different ribosomal RNA subspecies. Subsets of ribosomal RNA showed robust increases compared to control 48 h after the first session and other after the training period (C). Error bars shows 95\\% CI. Asterisk in C indicates robust differences between volume conditions (a 95\\% CI of pairwise differences not containing 0).", echo=FALSE}
...
reg_results <- 
  cbind(rbind(comb.m1$fixed,
      ...
      data.frame(coef = c("Intercept", 
                          ...
                          "Session 1-4\\textsuperscript{b}",
                          "Session 4-8\\textsuperscript{c}", 
                          "Session 8-12\\textsuperscript{d}", 
                          "De-training",
...
reg_results_s6 <- 
  ...
  mutate(Estimate = round(Estimate, 2), 
    ...
    footnote(alphabet = c("The dependent variable is total RNA levels (log).",
...
reg_results %>%
  ...
  mutate(Estimate = round(Estimate, 2), 
      ...
      kable(col.names = c("Coefficient", 
                        ...
                        "Lower 95\\% CI", 
                        "Upper 95\\% CI"), 
          caption = "Total RNA as a predictor of muscle growth.",
          format = "latex", 
          escape = FALSE,
          booktabs = TRUE) %>%
  
   footnote(alphabet = c("The dependent variable is $\\\\Delta$ Muscle thickness (mm)"), 
...
Finally, individual rates of increases in total RNA abundance predicted the magnitude of muscle growth, confirming the likely link between ribosomal biogenesis and muscle mass accretion 
[@RN1754;<!-- West 2016 correlation between total RNA and protein synthesis in C2C12-->
@RN2145]<!-- Millward 1973 relationship between RNA and protein synthesis in rat in rats in vivo constant infusion -->
and muscle hypertrophy
[@RN1644;<!-- Figueiredo 2015 Correlation between total RNA fold change and CSA hyp -->
